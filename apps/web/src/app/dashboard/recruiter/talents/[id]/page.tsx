...
interface Talent {
    id: string; firstName: string; lastName: string; email?: string; phone: string; country: string; city?: string; region?: string;
    candidateProfile?: {
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-[#14B53A]/30">
            ...
            <nav className="sticky top-[3px] z-40 border-b border-white/[0.06] bg-[#0a0a0a]/90 backdrop-blur-xl px-6 h-14 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <NotificationBell />
                    <Link href="/dashboard/recruiter/talents" className="text-sm text-gray-500 hover:text-white transition flex items-center gap-1.5">
                    ...
                    <motion.div variants={itemVariants} className="glass-card rounded-[32px] p-8 flex flex-col md:flex-row items-center md:items-start gap-8 relative overflow-hidden">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-4">
                            <div>
                                ...
                                <p className="text-[#FCD116] text-xl font-bold mt-1">{profile?.title || 'Candidat MaliTravail'}</p>
                            </div>
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-gray-400 font-medium">
                                <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                                    <MapPin size={14} className="text-[#CE1126]" /> {talent.city || talent.country}
                                </span>
                                <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                                    <Globe size={14} className="text-[#14B53A]" /> {profile?.isDiaspora ? 'Diaspora Malienne' : 'Au Mali'}
                                </span>
                                <span className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                                    <Calendar size={14} className="text-[#FCD116]" /> {profile?.availability === 'IMMEDIATE' ? 'Disponible Immédiatement' : 'Disponibilité à discuter'}
                                </span>
                            </div>
                        ...
                        <div className="md:col-span-2 space-y-8">
                            <motion.section variants={itemVariants} className="bg-white/[0.02] border border-white/[0.07] rounded-3xl p-8">
                                ...
                                <p className="text-gray-400 leading-relaxed whitespace-pre-line text-sm md:text-base">
                                    {profile?.summary || "Ce candidat n'a pas encore ajouté de résumé."}
                                </p>
                            </motion.section>
                            ...
                            <motion.section variants={itemVariants} className="space-y-4">
                                ...
                                <div className="space-y-4">
                                    {profile?.experiences && profile.experiences.length > 0 ? profile.experiences.map(exp => (
                                        <div key={exp.id} className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6 relative">
                                            ...
                                            <div className="pl-4">
                                                ...
                                                <p className="text-gray-400 font-medium text-sm mb-2">{exp.company} · {exp.type}</p>
                                                <p className="text-xs text-gray-500 mb-4 bg-white/5 px-2 py-1 rounded-md inline-block">
                                                    ...
                                                    {exp.endDate ? new Date(exp.endDate).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' }) : "Aujourd'hui"}
                                                </p>
                            ...
                            <motion.section variants={itemVariants} className="space-y-4">
                                <h2 className="text-lg font-bold flex items-center gap-2 text-[#CE1126] ml-2"><GraduationCap size={20} /> Éducation et Formations</h2>
